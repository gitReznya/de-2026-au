# ДЭ-2026 AU — сетевое и системное администрирование

> Учебный разбор демонстрационного экзамена 09.02.06.  
> Вариант адаптирован под домен `au-team.irpo`, адресацию `192.168.*`, VLAN `120 / 220 / 888` и GRE `10.10.10.0/30`.
...
Это структурированный учебный репозиторий для подготовки к ДЭ-2026 по специальности **09.02.06 «Сетевое и системное администрирование»**.

Идея простая: сначала смотришь таблицы адресации, потом идёшь по модулям и выполняешь команды по конкретным машинам.
...
1. Открой [`TABLES.md`](./TABLES.md) и проверь адресацию.
2. На каждой машине проверь интерфейсы:

```bash
ip -br a
```

3. Если интерфейсы отличаются от примера, замени переменные в начале командных файлов.
4. Выполняй модули по порядку:

```text
Модуль 1 → Модуль 2 → Модуль 3
```

5. После каждого блока запускай проверки из `checks.md`.
